...
{
  pdf(file='figuras/OndaletasEscala.pdf', height=3.2, width=7.8)
  par(mfrow=c(2,2), las=1, cex=0.9, mgp=c(2.6,1,0), mar=c(4,3.7,2,2.5))
  # N = 1
...
# Figura 3: Regra de shrinkage bayesiana
{
  x <- seq(-5.95, 5.95, 0.05)
  a <- c(0.2, 0.5, 0.7, 0.9)
  t <- c(5, 20, 60, 180)
  
  ## Figura 3 (a): Variando t para alpha=0.8
  pdf(file='figuras/ShrinakgeLogistica1.pdf', height=3.9, width=5.75)
  ...
  legend('topleft',  bty='n', lwd=2, col=1:4, cex=.9,
         legend=c(expression(tau == 5), expression(tau == 20),
                  expression(tau == 60), expression(tau == 180)))
  dev.off()
  
  ## Figura 3 (b): Variando alpha para t=50
  pdf(file='figuras/ShrinakgeLogistica2.pdf', height=3.9, width=5.75)
  par(mar=c(4.3, 4.3, 1.5, 1), cex=1.3, las=1)
  plot(x=1, type='n', xlab='d', ylab=expression(delta(d)),
       xlim=c(-5.5,5.5), ylim=c(-5.5,5.5))
  # abline(v=0, h=0, lty=1)
...
# Figura 5: amostra, coeficientes empíricos e estimados e função recuperada
{
  ## Figura 5 (a): amostra
  pdf(file='figuras/ExemploAmostra.pdf', height=3.7, width=6.75)
  ...
  ## Figura 5 (b): coeficientes empíricos
  pdf(file='figuras/ExemploCoefEmp.pdf', height=3.7, width=6.75)
  ...
  ## Figura 5 (c): coeficientes estimados
  pdf(file='figuras/ExemploCoefEst.pdf', height=3.7, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.15, las=1)
  ...
  ## Figura 5 (d): função recuperada
  pdf(file='figuras/ExemploRecup.pdf', height=3.7, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.15, las=1)
  f_hat <- wr(d_hat)
...
# Figura 6: Amostra agregada
  pdf('figuras/ExemploAmostraFuncional.pdf', height=2.9, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1, las=1)
  plot(1, xlab='x', ylab='y', ylim=range(A), xlim=c(0,1), type='n')
  for (i in 1:I) lines(x, A[,i], col=i)
  dev.off()
  
# Figura 7: Funções recuperadas
  ## Figura 7 (a): bumps recuperada
  pdf('figuras/ExemploFunRecup1.pdf', height=3.5, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.15, las=1)
  ...
  ## Figura 7 (b): doppler recuperada
  pdf('figuras/ExemploFunRecup2.pdf', height=3.5, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.15, las=1)
...
## Figura 8 (a): Amostra
  pdf('figuras/ExemploErroPositivoAmostra.pdf', height=3.6, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.1, las=1)
  ...
  delta_d <- ram_gamma(p, S_1=NULL, d, n_ite=100000, alpha=alpha, tau=tau,
...
## Figura 8 (b): Função estimada
  pdf('figuras/ExemploErroPositivoEstimativa.pdf', height=3.6, width=6.75)
  par(mar=c(4.1, 4.1, 1.5, 1), cex=1.1, las=1)
...
## Figura 9: Amostra de curvas agregadas do Tecator
...
## Figura 10: Funções constituintes
  # Figura 10 (a): componente de gordura
  ...
  # Figura 10 (b): componente de água
  ...
  # Figura 10 (c): componente de proteína
...
#                   2,3,4,
#                   2,3,4), 5, byrow=T))
#   par(mar=c(4.1, 4.5, 3, 1), mgp=c(3,.75,0), las=1)
#   plot(fun_tec, ylab='Absorbância', xlab='Comprimento de onda (nm)', main='Amostra')
#   plot(x, alpha_hat_bayes[,1], type='l', lwd=2, main='Gordura',
#        ylab=expression(hat(alpha)[1](t)), xlab='Comprimento de onda (nm)')
#   plot(x, alpha_hat_bayes[,2], type='l', lwd=2, main='Água',
#        ylab=expression(hat(alpha)[2](t)), xlab='Comprimento de onda (nm)')
#   plot(x, alpha_hat_bayes[,3], type='l', lwd=2, main='Proteína',
#        ylab=expression(hat(alpha)[3](t)), xlab='Comprimento de onda (nm)')
#   # par(mar=c(5.3, 6.2, 1, 1), mgp=c(4,.75,0), cex.lab=1.9, cex.axis=1.3)
# }

# Figura 13: Funções de teste (apêndice)
{
  x <- (1:1024)/1024
